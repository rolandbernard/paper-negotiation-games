...
\begin{figure}[ht!]
    ...
    % --- Main caption for the entire figure ---
    \caption{Reasoning tokens for German tasks}
...
MODEL OUTPUTS:
\begin{enumerate}
    \item \textbf{Dominance}: We noticed two dominance patterns in our game. 1) One player will attempt to dominate the other by insisting on a proposal. We measure this as stubbornness which is the relative frequency of repeated proposals in an instance. Figure~\ref{fig:air-balloon-stubbornness-all}. Stubbornness increases significantly on instances with opposing goals and is typically higher in player 1. Reasoning can mitigate stubbornness (GPT-5-mini, GPT-5, Nemotron), but it can also yield high stubbornness in models which are not as good at counterpart modelling and common ground finding (GPT-OSS, Qwen3). In the latter models players can find individual knapsack solutions, but they do not perform general reasoning that allows them to resolve tension as well.
    2) We noticed that in some (inferior) models players will take on different roles, one being the proposer (more proactive) and the other being a refuser of critiquer (more passive). We measure this by the rate average rate of alternations of proposals in an instance Table~\ref{tab:air-balloon-alternation}. The alternation rate measures how often the proposer switches between the two players from one proposal to the next. It is the share of consecutive proposals where a different player makes an offer. It ranges from 0 (one player proposes every time) to 1 (they perfectly take turns). Commercial models and reasoning models typically get higher alternation rate because players advocate better for themselves
    \item \textbf{Collaboration/Common Ground}: We noticed that some of our models use particular collaborative strategies. One such strategy is to continuously craft the deal in a coherent fashion such that players agree on more and more items throughout the negotiation. Each proposal works as a blueprint to the next This type of collaboration is apparent in commercial models and is enhanced by enabling reasoning on those models. Cf. Figure \ref{fig:air-balloon-gpt-5-subs}-\ref{fig:air-balloon-dschatv3.1-subs} which depicts the average substitutions made at each proposal ordered. The best models GPT-5 and GPT-5 mini have high initial disagreement and rapid descent afterwards because they bargain efficiently. This pattern is enhanced by reasoning. It also comes with pitfalls as it largely explains why p1 get a better score than p2 on average. P1 gets to sets the stage so to speak and p2 just follows.
    \item \textbf{Anchor positioning}: High/low initial disagreement, again cf. Figure \ref{fig:air-balloon-gpt-5-subs}-\ref{fig:air-balloon-dschatv3.1-subs}
\end{enumerate}
...
REASONING TRACES
\begin{enumerate}
    \item \textbf{knapsack reasoning} Reasoning models are basically always better at solving the Knapsack problem than their counterparts. Typically they work in different phases 1) apply some greedy heuristic to the problem instance and order the items (e.g. by value-to-weight ratio). 2) add items to candidate proposal until weight limit is reached. 3) given the proposal perform search by looking at possible changes to the candidate proposal. Llama distill and nemotron reasoning get stuck in error correction loops in one of the three stages. Models like GPT-OSS use the reasoning traces for following the correct syntax which the Nemotron and R1 distill do not and hence get worse scores although their quality scores are ok.
    \item \textbf{Counterpart modelling}. models do try to model their counterpart although there is no more sophisticated modelling than noting that the other player has made proposals containing XYZ items and hence they may value them. Llama confuses itself with the other player on occasion when reprompted. Figure \ref{tab:air-balloon-pareto-adherence} reports the per-instance Pareto-adherence rate, i.e. the per-instance relative frequency of proposals which lie on the Pareto front. Because Pareto-efficient proposals require aligning both agents' payoffs --albeit not necessarily in a balanced way-- higher adherence may indicate stronger counterpart and self-modelling, i.e. the ability to infer the other player's item valuations and adjust one's own search for possible deals accordingly.
    \item \textbf{Loops}: Nemotron and Llama frequently encounter error correction loops and are not efficiently searching the problem space in step 3). Nemotron reaches sort fixed points where it repeats the exact same sentence until context is full. 
\end{enumerate}
